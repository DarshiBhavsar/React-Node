...
const schema = require("./schemaValidate1");
...
const create = async (req, res) => {
  try {
    ...
    const userBranches = await BranchModel.find({
      $or: [{ main_user_id: loggedInUser._id }],
      enabled: true,
      removed: false,
    })
      .select("_id name")
    ...
    body.subId = loggedInUser instanceof AdminModel ? decoded.id : loggedInUser.subId || decoded.id;
